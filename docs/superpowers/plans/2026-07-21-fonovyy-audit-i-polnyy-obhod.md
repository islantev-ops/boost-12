...
Поэтому отпечаток считается **по составу, а не по порядку**, и **только по
основному содержимому**: обрезаем вход до 300 КБ, берём `main`/`article`
(иначе `body` без `header`, `footer`, `nav`, `aside`), считаем частоты
`tag.class` и берём 15 самых частых. Единичный бейдж в топ-15 не попадает,
меню исключено из области счёта, стоимость ограничена обрезкой.

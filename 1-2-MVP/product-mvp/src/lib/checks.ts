    ...
    summary:
      verdict === 'violation'
        ...
        : verdict === 'ok'
          ...
          : 'Ссылку на Политику в коде найти не удалось, но место, где она обязана быть, нам не видно. Требуется ручная проверка в браузере.',
  };
}
  ...
  return {
    ...
    summary: !hasDoc
      ? verdict === 'violation'
        ...
        : 'Отдельный документ с согласием не найден, но место, где ссылка обязана быть, нам не видно. Требуется ручная проверка.'
      : composition?.vote === 'violation'
    ...
    summary:
      verdict === 'violation'
        ...
        : verdict === 'ok'
          ...
          : 'Документ не найден, но место, где ссылка обязана быть, нам не видно. Требуется ручная проверка.',
  };
}
    ...
    return {
      ...
      summary:
        verdict === 'violation'
          ...
          : 'Плашку рисует скрипт — в исходном коде её нет, поэтому состав проверить нечем. Это единственный случай, когда пункт уходит в ручную проверку: нужен просмотр страницы в браузере.',
    };
  }

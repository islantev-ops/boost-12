...
/**
 * Честная формулировка полноты обхода. Обход мог закончиться сам
 * (`coverage.complete`), но отбросить десятки однотипных страниц по лимиту
 * «не более 5 на группу» (см. fingerprint.ts) — тогда «сайт обойдён
 * полностью» буквально ложно: часть страниц не осмотрена, а отброшена как
 * предполагаемый дубликат. Вердикт это не меняет (пропущенные страницы —
 * копии уже осмотренных по тому же адресу и разметке), меняется только
 * формулировка. Один хелпер вместо копипасты по местам, где раньше стояла
 * фраза «сайт обойдён полностью».
 */
function coverageCompleteness(s: SiteSnapshot): string {
  return s.coverage.skippedByTemplate === 0
    ? 'сайт обойдён полностью'
    : `обход завершён, ${s.coverage.skippedByTemplate} однотипных страниц пропущено как дубликаты`;
}
...
function absenceGateFactor(s: SiteSnapshot, found: boolean): Factor {
  return {
    ...
    detail: canProveAbsence(s)
      ? `HTML и подвал отдаются сервером, ${coverageCompleteness(s)} — отсутствие ссылки показательно.`
...
function check7(s: SiteSnapshot): CheckResult {
  ...
  if (pdForms > 0) {
    ...
    factors.push({
      ...
      vote: nothingFound(s),
      detail: canProveAbsence(s)
        ? `Форм сбора персональных данных на осмотренных страницах не найдено, ${coverageCompleteness(s)}.`
        : absenceUnknownReason(s),
    });
  }
      ...
      : noCheckbox.length
        ...
        : pdForms === 0
          ? `Форм сбора персональных данных не найдено. ${
              canProveAbsence(s)
                ? `${capitalize(coverageCompleteness(s))}, поэтому вывод достоверен. Сохранение факта согласия всё равно определить нельзя: по внешнему виду сайта оно не видно.`
                : absenceUnknownReason(s)
            }`

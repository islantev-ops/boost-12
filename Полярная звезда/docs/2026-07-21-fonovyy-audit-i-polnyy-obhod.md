...
**Правка от 2026-07-21 по итогам ревью.** Первая версия считала отпечаток как
список первых 400 элементов ПО ПОРЯДКУ. Ревью доказало замерами, что так
работать не будет:
1. Один необязательный блок (бейдж «Скидка») сдвигает список — однотипные
   карточки получают разные отпечатки, и выборка не срабатывает вовсе.
2. Меню из 450 пунктов занимает все 400 позиций — главная и карточка товара
   получают ОДИН отпечаток, и мы пропускаем страницы (та самая история с
   `/career/`).
3. Ограничение в 400 элементов не ограничивает стоимость: `cheerio.load` и
   селектор всё равно проходят документ целиком (100k элементов ≈ 1,1 с).

Вторая версия (15 самых частых `tag.class`) тоже провалилась: у одиночной
карточки уникальных элементов всего 4-5, все по одному разу, и бейдж попадает
в отпечаток наравне с остальными.

**Итоговое правило подобрано перебором.** Отдельный стенд проверил 3 варианта
на 13 сценариях (отчёт: `.superpowers/sdd/fingerprint-lab.md`). Победитель —
«схлопывание редких»: считаем, сколько раз встречается каждая пара
(тег, первый класс); если пара встретилась **2 и более раз** — кладём в
отпечаток `tag.class`, если один раз — кладём только `tag`. Отпечаток — это
МНОЖЕСТВО таких ключей, без порядка и без счётчиков.

Почему это работает: единичный `span.discount-badge` схлопывается в `span`,
который на карточке и так есть, — множество не меняется. А повторяющиеся
элементы каркаса (`li.menu__item` × 450, `div.card` × 20) сохраняют класс и
продолжают различать типы страниц.

**Известная граница (осознанно принята).** Ни один из трёх вариантов не прошёл
все 13 сценариев. Победитель ошибается в двух: когда у одной карточки есть
целый необязательный БЛОК (секция отзывов), и когда две новости сильно разной
длины. В обоих случаях ошибка идёт **в безопасную сторону** — однотипные
страницы считаются разными, и мы качаем лишнее. Опасное направление — слияние
разных страниц в одну группу и пропуск непроверенных — вариант проходит во
всех сценариях. Причина ограничения структурная: функция видит одну страницу,
без статистики по сайту и без адреса.

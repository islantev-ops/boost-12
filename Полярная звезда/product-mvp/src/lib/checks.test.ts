...
/**
 * Финальное ревью, Important 2: обход мог завершиться сам (`complete: true`),
 * но отбросить десятки однотипных страниц по лимиту «не более 5 на группу»
 * (см. fingerprint.ts). «Сайт обойдён полностью» в этом случае буквально
 * ложно — часть страниц не осмотрена, а отброшена как предполагаемый дубликат.
 * Вердикт при этом не меняется (пропущенные страницы — копии осмотренных по
 * тому же адресу и разметке), меняется только формулировка.
 */
const SKIPPED_TEMPLATE_COVERAGE = {
  crawled: 40, discovered: 40, skippedByTemplate: 12, skippedByLimit: 0,
  complete: true, stopReason: 'done' as const,
};

test('check3: обход завершён, но однотипные страницы пропущены — «обойдён полностью» не пишем, число пропущенных называем (Important 2)', () => {
  const s = snapshot(RU);
  const partial: SiteSnapshot = { ...s, coverage: SKIPPED_TEMPLATE_COVERAGE };
  const f = check3of(partial);
  const text = f.factors.map((x) => x.detail).join(' ');
  assert.doesNotMatch(text, /обойдён полностью/, 'часть страниц пропущена, а не осмотрена — так писать нельзя');
  assert.match(text, /12/, 'число пропущенных однотипных страниц должно быть названо');
  assert.equal(f.verdict, 'violation', 'вердикт не должен меняться из-за формулировки охвата');
});

test('check7: обход завершён, но однотипные страницы пропущены — summary без «обойдён полностью», с числом пропущенных (Important 2)', () => {
  const s = snapshot(RU); // CLEAN_HTML — без единой формы
  const partial: SiteSnapshot = { ...s, coverage: SKIPPED_TEMPLATE_COVERAGE };
  const f = check7of(partial);
  assert.doesNotMatch(f.summary, /обойдён полностью/, 'часть страниц пропущена, а не осмотрена — так писать нельзя');
  assert.match(f.summary, /12/, 'число пропущенных однотипных страниц должно быть в summary');
});

test('check3: при skippedByTemplate = 0 формулировка прежняя — «обойдён полностью» (регресс, Important 2)', () => {
  const f = check3of(snapshot(RU)); // coverage по умолчанию: skippedByTemplate 0, complete true
  const text = f.factors.map((x) => x.detail).join(' ');
  assert.match(text, /обойдён полностью/, 'без пропущенных страниц формулировка не должна меняться');
});

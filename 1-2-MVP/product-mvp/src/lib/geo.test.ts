...
/* ─────────────────── IMPORTANT 1: ip указывает ровно на измеренный адрес ─────────────────── */

test('несколько A-записей, RDAP молчит про один — ip указывает только на реально проверенный адрес (IMPORTANT 1, регресс)', async () => {
  // Воспроизведение из отчёта о финальном ревью: RDAP молчит про первый
  // адрес (его страну никто не узнавал) и отвечает DE про второй,
  // ipwho.is подтверждает DE. Раньше ips (все checked) и есть тот список,
  // что печатался в тексте обвинения целиком — из-за чего непроверенный
  // первый адрес звучал как измеренный. Теперь geo.ts обязан отдать
  // отдельно, к какому именно адресу относятся country/netname.
  const fact = await resolveHosting('https://example.com/', deps({
    resolve4: async () => ['1.1.1.1', '2.2.2.2'],
    fetchJson: async (url) => {
      if (url.includes('rdap.org')) {
        return url.includes('1.1.1.1') ? null : { country: 'DE', name: 'HETZNER-NET' };
      }
      return { success: true, country_code: 'DE' };
    },
  }));
  assert.deepEqual(fact.ips, ['1.1.1.1', '2.2.2.2'], 'все проверенные адреса по-прежнему сохраняются в ips');
  assert.equal(fact.ip, '2.2.2.2', 'country/netname получены только по этому адресу — он и есть представитель');
  assert.equal(fact.country, 'DE');
  assert.equal(fact.netname, 'HETZNER-NET');
});

/* ─────────────────── MINOR 1: country/country_code не двухбуквенный код ─────────────────── */

test('RDAP прислал полное название страны, а не код — считаем, что страну не назвали (MINOR 1, регресс)', async () => {
  // RFC 9083 требует двухбуквенный ISO-код. "RUSSIAN FEDERATION" этому не
  // соответствует; раньше это принималось как есть и давало
  // самоопровергающуюся фразу «Сайт размещён за пределами РФ… страна RUSSIAN
  // FEDERATION». Правильный итог — как будто реестр страну не назвал вовсе.
  let ipwhoCalls = 0;
  const fact = await resolveHosting('https://example.ru/', deps({
    fetchJson: async (url) => {
      if (url.includes('rdap.org')) return { country: 'RUSSIAN FEDERATION', name: 'SOME-NET' };
      ipwhoCalls += 1;
      return { success: true, country_code: 'US' };
    },
  }));
  assert.equal(fact.country, null, 'не двухбуквенный код — не принимаем, реестр «страну не назвал»');
  // RDAP ответил и назвал сеть без страны — второй источник в одиночку не
  // обвиняет (правка 2026-07-16), поэтому ipwho.is в этом случае не спрашиваем.
  assert.equal(ipwhoCalls, 0);
});

test('ipwho.is прислал не двухбуквенный country_code — geoCountry не берём (MINOR 1, регресс)', async () => {
  const fact = await resolveHosting('https://example.com/', deps({
    fetchJson: async (url) =>
      url.includes('rdap.org')
        ? { country: 'DE', name: 'HETZNER-NET' }
        : { success: true, country_code: 'Germany' },
  }));
  assert.equal(fact.geoCountry, null, 'не двухбуквенный код от ipwho.is — geoCountry не подтверждён');
  assert.deepEqual(fact.confirmedBy, ['rdap'], 'geoCountry не принят — ipwho.is не в числе подтвердивших');
});
